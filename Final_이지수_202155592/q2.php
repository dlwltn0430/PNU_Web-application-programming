<?php
    ...
    // Connect to MySQL
    $mysqli = new mysqli("localhost", "root", "dlwltn0430", "202155592", "3307");
    ...
    {
        // 중복 검사 먼저 수행
        $sql = "select * from contact where name='".$_POST['txt_name']."'";
        if ( $result = $mysqli->query($sql) ) 
        {
            if ($result->num_rows > 0)
            {
                $err = "이미 등록된 이름입니다. 다른 이름을 사용해주세요.";
            }
            else
            {
                $sql = "insert into contact set name = '".$_POST['txt_name']."', tel = '".$_POST['txt_tel']."', email = '".$_POST['txt_email']."', memo = '".$_POST['txt_memo']."'";
                $mysqli->query($sql);
            }
            $result->close();
        }
    ...
    else if ($mode == "delete")
    {
        ...
        if ( $result = $mysqli->query($sql) ) 
        {
            ...
            {
                $sql = "delete from contact where name = '".$_POST['txt_name']."'";
                $mysqli->query($sql);
            }
            $result->close();
        }
    ...
    $select_sql = "select * from contact order by name asc";
    $data_arr = array();
    if ( $result = $mysqli->query($select_sql) ) 
    {
        while($data = $result->fetch_assoc())
        {
            $data_arr[] = array("name"=>$data['name'], "tel"=>$data['tel'], "email"=>$data['email'], "memo"=>$data['memo']);
        }
        $result->close();
    }
?>
